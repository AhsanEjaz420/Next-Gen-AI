...
from app.schemas.blog_seo import BlogSEORequest
from app.services.blog_seo_service import generate_blog_topics
...
router = APIRouter(prefix="/blog-seo", tags=["Blog Topic + SEO Generator"])


@router.post("/generate")
async def generate_blog_seo(
    request: BlogSEORequest,
    ...
    async with SubscriptionService.credit_session(db, user, amount=1) as sub:
        # 2. Async call
        result = await generate_blog_topics(request, profile)
